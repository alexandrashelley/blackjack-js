  ...
  startGame() {
    ...
    if (welcome != null) {
      ...
      while (shouldPlayFlag) {
        ...
        if (this.game.hand.getHandValue() > 21) {
          console.log(
            `Uh-oh. You pulled the ${this.game.hand
              .showHand()
              .at(-1)}. You scored ${this.game.hand.getHandValue()}.`
          );
        } else {
          console.log(
            `Congrats! You scored ${this.game.hand.getHandValue()}.`
          );
        }
  ...
  giveCurrentHand() {
    console.log(
      `Your current hand is: ${this.game.hand
        ...
        .join(
          ", "
        )}. You have a total of ${this.game.hand.getHandValue()} points.`
    );
  }
}
